...
class Insights:
    ...
    def calculer_statistiques(self, groupby_col):
        ...
        # Distribution des types locaux et prix médian au m²
        df_type_local_dist = self.df.groupby(groupby_col)['type_local'].value_counts().unstack(fill_value=0).reset_index()
        df_type_local_median = self.df.groupby([groupby_col, 'type_local'])['prix_m2'].median().reset_index()  
        df_type_local_median.rename(columns={'prix_m2': 'median_prix_m2_local'}, inplace=True) 
